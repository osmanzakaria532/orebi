...
import { apiConation } from "../../../counterSlice";
...
const Product = ({
  ...
  let previousData = useSelector((pvsData) => pvsData.counter.value);
  //   console.log(previousData);
  let disPatch = useDispatch();
  let abc = () => {
    if (previousData.find((el) => el.id == ForAdToCard.id)) {
      let arr = previousData.map((el) => {
        if (el.id == ForAdToCard.id) {
          return {
            ...el,
            countt: el.countt + 1,
          };
